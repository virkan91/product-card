...
const Electronics = () => {
  const electronics = useSelector((state) => state.cart.data).filter(
    (e) => e.type === "electronics"
  );
  ...
  return (
    <div className="p-[20px]">
      ...
      <div className="flex justify-center py-3 items-center gap-[10px]">
        ...
        <div className="flex">
          <p>{start}</p>-
          <p>{end > electronics.length ? electronics.length : end}</p>
        </div>
        <button
          onClick={() => {
            setStart(start + 10);
          ...
          disabled={end > electronics.length ? true : false}
          className={` p-1 border rounded-full ${
            end > electronics.length ? "text-gray-400" : "hover:bg-gray-100"
          }  text-[20px]`}
        >
